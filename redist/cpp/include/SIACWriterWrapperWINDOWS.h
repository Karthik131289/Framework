...
#include "SIACCanonicalOutputWriter.h"
	typedef int (__stdcall * CF_SetDecimalRangePtr)(unsigned short , unsigned short );
...
#define MAX_SIAC_BUF_LEN 904999
	...
	TagInfo tagInfo;
	///ADDED ///////////////////////
	CF_SetDecimalRangePtr setDecimalRange;
	HMODULE mod;
	/////////////////////////////////
public:
	SIACWriterWrapper() : outCan(CFO_NO_NULL_TERMINATION) {
		...
		tagInfo.type = 0;	
		///ADDED ///////////////////////
		mod = LoadLibrary("CanonicalAPI.dll");
		setDecimalRange = (CF_SetDecimalRangePtr)GetProcAddress(mod, "?CF_SetDecimalRange@OutgoingCanonical@@QAEHGG@Z");
	 /////////////////////////////////
	}
	///ADDED ///////////////////////
	~SIACWriterWrapper() {
 ...
 /////////////////////////////////
	...
	void initMessage() {
		...
		///ADDED ///////////////////////
		OutgoingCanonical* canPtr =  &outCan;
		void * thisPtr =  this;
		_asm {
			mov ECX, canPtr
		}
		result = setDecimalRange(0, 8);
		_asm {
			mov ECX, this
		}
		/////////////////////////////////
		...
		if(tagInfo.primitive) {
			if(tagInfo.isDouble) {
			  int option = (lastField ? CFO_LAST_FIELD : 0);
				result = outCan.CF_PutNextField(option, tagInfo.tag, (void*)&tagInfo.doubleValue, (unsigned int) sizeof(tagInfo.doubleValue), &retnBufLen);
